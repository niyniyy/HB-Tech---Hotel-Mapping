...
def _clean_postal_code(value) -> str | None:
    """
    Normalise a postal code to digits only.

    pandas reads a numeric postal column as float, so `str(value)` yields
    "110001.0". Three of the five suppliers were storing codes that way and two
    were storing "110001", which made every cross-supplier comparison fail on
    formatting alone — agreement between records for the same hotel measured
    45.9% before this fix and 87.7% after.

    Non-numeric junk (one supplier ships state names in this field) becomes NULL
...
class ImportService:
    ...
    f"Batch {batch_num + 1}/{num_batches}: inserted {len(ids)} rows")
        ...
        if inserted_ids:
            ...
            # Word frequencies are a property of the whole corpus, so new
            # records change what counts as a distinctive name. Rebuilt here
            # rather than during mapping: a stale table would silently score
            # every new supplier's names against the old feed's vocabulary.
            await self.db.execute(text("SELECT refresh_name_token_df();"))
    ...
    def _clean_row(self, row: pd.Series, supplier_name: str) -> dict | None:
        ...
        return {
            ...
            # Computed here, never taken from the feed: normalized_name is
            # supplier-supplied and does not strip the city, so it cannot serve
            # as a blocking key.
            ...
            "postal_code": _clean_postal_code(row.get("postal_code")),
            "latitude": lat,
            "longitude": lon,
            "star_rating": star,
            "raw_json": raw_json,
