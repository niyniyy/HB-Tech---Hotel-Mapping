...
class SupplierHotel(Base):
    ...
    # The two computed blocking keys. Unlike normalized_name, which is whatever
    # the supplier shipped, these are derived here from hotel_name.
    #   core_name    core_hotel_name(hotel_name, city, state) — city removed
    #   strict_name  normalize_hotel_name(hotel_name)         — city retained
    core_name = Column(Text, nullable=True, index=True)
    strict_name = Column(Text, nullable=True, index=True)
